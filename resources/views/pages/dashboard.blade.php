...
<section>
    <div class="wrap dashboard-grid">
        <div class="contact-form">
            ...
            <h2>{{ $zionUser['name'] ?? 'Zion user' }}</h2>
            <p class="muted-text">Logged in through dev Zion Shipping and stored in Kay Paolo session.</p>
            <dl class="session-list">
                <div><dt>Role</dt><dd>{{ $zionUser['role']['name'] ?? 'User' }}</dd></div>
                <div><dt>Role ID</dt><dd>{{ $zionUser['role_id'] ?? '-' }}</dd></div>
                <div><dt>Email</dt><dd>{{ $zionUser['email'] ?? '-' }}</dd></div>
                <div><dt>Account</dt><dd>{{ $zionUser['account_number'] ?? '-' }}</dd></div>
            </dl>
        </div>
